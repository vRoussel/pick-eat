<template>
    ...
    >
        <picture v-if="recipe.image" class="relative">
            <source
                type="image/avif"
                :srcset="recipe.image.replace('upload', 'upload/c_limit,h_512,w_512,f_avif')"
            />
            <source
                type="image/webp"
                :srcset="recipe.image.replace('upload', 'upload/c_limit,h_512,w_512,f_webp')"
            />
            <img
...
export default {
    ...
    computed: {
        ...
        cart_svg() {
            return this.cartStore.hasRecipe(this.recipe.id)
                ? this.icons.cart
                : this.icons.cart_outline
        },
        is_vege() {
    ...
    methods: {
        toggleFavorite(recipe) {
            if (this.authStore.is_logged_in) this.foodStore.toggleFavorite(recipe)
            else this.notifStore.show_error('Vous devez vous connecter pour utiliser les favoris')
        },
        openRecipe(id) {
