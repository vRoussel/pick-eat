...
-->
    ...
    >
        ...
        <div class="flex flex-wrap sm:flex-nowrap gap-y-12 gap-x-4 md:gap-x-6">
            ...
            <div
                class="flex flex-col basis-full sm:basis-1/2 justify-between items-center mx-auto gap-y-2 sm:gap-y-0"
            >
                <p
                    ref="recipe_name"
                ...
                <p class="space-x-4">
                    <span
                        class="icon inline-flex items-center gap-x-1 text-sm sm:text-lg md:text-xl lg:text-2xl"
                ...
                <p class="text-xs sm:text-sm italic text-center">
                    Ajoutée par {{ recipe.author_name }} <br /><router-link
                        class="link-primary"
        ...
        <div
            class="flex flex-wrap sm:flex-nowrap gap-x-4 sm:gap-x-6 md:gap-x-8 items-start gap-y-12"
        >
            <table class="table table-compact basis-2/5 md:basis-1/3 shrink-0 grow sm:grow-0">
                <thead>
                    <tr class="text-center">
                        ...
                        >
                            <span v-if="recipe.n_shares > 0" class="icon inline-flex items-center"
                                >Ingrédients ({{ recipe.n_shares }}
                                <Icon class="pl-0.5" :icon="icons.person" />)</span
                            >
            ...
            <table class="table table-compact table-fixed grow w-full">
                ...
                <tbody>
                    <tr v-for="(step, index) in recipe.instructions" :key="index" class="border-0">
        ...
        <blockquote v-if="recipe.notes" class="pl-4 sm:pl-6 self-center w-full">
...
export default {
    ...
    computed: {
        ...
        allowed_to_modify() {
            return (
                this.authStore.is_logged_in &&
                (this.authStore.account.id == this.recipe.author_id || this.authStore.is_admin)
            )
        },
    },
    mounted() {
    ...
    methods: {
        ...
        image(format = null) {
            ...
            if (this.recipe == null || this.recipe.image === '') return this.icons.camera
            else return this.recipe.image.replace('/upload', replacement)
        },
    },
}
</script>
