<template>
    <router-link ref="recipe_name" v-tooltip="overflown ? recipe_name : null" class="text-end truncate grow link-hover" :to="'/recipe/' + recipe_id">{{ recipe_name }}</router-link>
    <span> x </span>
    <number-input class="basis-10 grow-0" :modelValue="shares" @update:modelValue="(val) => this.cart.updateShares(recipe_id, val)" :min="0"/>
    <span> parts</span>
    <button class="btn btn-circle btn-xs btn-outline grow-0" tabindex="-1" type="button" @click="this.cart.removeRecipeWithId(recipe_id)">
