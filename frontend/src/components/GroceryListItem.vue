...
onMounted(async () => {
    await nextTick()
    overflown.value = isOverflown(recipe_name_input_el)

})
</script>

<template>
    <router-link ref="recipe_name_input_el" v-tooltip="overflown ? recipe_name : null"
        class="text-end truncate grow link-hover" :to="'/recipe/' + recipe_id" tabindex="-1">
        {{ recipe_name }}
    </router-link>
    <span> x </span>
